...
- выбор товара -> карточка товара через `GET /products/:id`
...
- ФИО получателя в Telegram checkout должно содержать минимум два слова; односоставное значение не отправляет заявку и просит пользователя исправить ввод
